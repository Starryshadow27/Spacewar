...
function createAsteroidBelt() {
    roids = [];
    var x, y;
    for (var i = 0; i < asteroids + level; i++) {
...
function destroyAsteroid(index) {
    ...
    if (r == (initial_size / 2)) { 
        roids.push(newAsteroid(x, y, (initial_size / 8)));
        roids.push(newAsteroid(x, y, (initial_size / 8)));
        score += 25;
    } else {
        score += 100;
...
function newAsteroid(x, y, r) {
    ...
    var roid = {
        ...
        r: r,
        vert: Math.floor(Math.random() * (avg_shape + 1) + avg_shape / 2)
    };
    return roid;
}
...
function shootBullet() {
    if (spaceship.canShoot && spaceship.bullets.length < maxbullets) {
